...
sc['scene'].add( new THREE.AmbientLight( 0x666666, 0.1 ) );
/*
...
function init() {
   ...
   // scene = new THREE.Scene();
    ...
    for (var i = 0; i < 10; i++) {
        ...
        cube.position.set(i*1.2+30, 50, 4);
...
//    camera.lookAt(cubes[5].position);
   // sc['scene'].add(camera); // not required
    ...
    var terrainGeo = new THREE.PlaneGeometry(500, 500);
    var terrainMaterial = new THREE.MeshLambertMaterial({ color: 0xc0c0a0 });
    var terrain = new THREE.Mesh(terrainGeo, terrainMaterial);
    terrain.position.z = 6; // CHANGED
    terrain.position.y = 50; // CHANGED
    terrain.position.x = 40; // CHANGED
   ...
   //  sc['scene'].add(terrain);
    ...
    light.shadowCameraLeft = -2000; // CHANGED
	light.shadowCameraRight = 2000; // CHANGED
	light.shadowCameraTop = 2000; // CHANGED
	light.shadowCameraBottom = -2000; // CHANGED

    light.position.set(-60, 20, 100); // CHANGED
   // light.position.set(0, 0, 100); // CHANGED
    sc['scene'].add(light);
